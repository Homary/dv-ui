  ...
  <div class="img-uploader"
    ...
    <div v-if="hasImages" class="img-uploader-preview-list">
      <div v-for="(data,index) in imageDataList" class="img-uploader-preview">
        ...
        <i class="OD dv-icon-close img-uploader-delete-btn" @click="deleteImg(index)"></i>
      </div>
    </div>
...
//        this.onChange && this.onChange(files)
        ...
        this.uploadFile(files[0]).then((response) => {
            let dom = this.$refs['uploader'];
            this.$emit('input', JSON.parse(response).url, dom);
            this.$emit('change', JSON.parse(response).url, dom);
        });
        this.preview(files);
      },
      uploadFile (file) {
        ...
        return new Promise((resolve, reject) => {
          var request = new XMLHttpRequest();
            request.open('post', this.options.upload_url);
